...
class Suggested extends React.Component {
    ...
    constructor(props) {
      ...
      render() {
        return (
            <div>
              <SectionHeader firstHeader="Suggested" secondHeader="Moves to 'Discussed' when has 1000 upvotes" />
  
              {this.state.list.map(item => (
              <div key={item.id} className="single-accordion-section">
                  <Voting />
                  <div className="progress-and-accordion-section">
                      <Collapsible triggerClassName="main-trigger suggested" transitionTime={400} trigger={item.mainTrigger}>
